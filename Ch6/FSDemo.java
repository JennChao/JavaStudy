...
class FailSoftArray {
    ...
    // public int length; is equal to int length;
...
class FSDemo {
    public static void main(String args[]) {
        ...
        // show quite failures
        System.out.println("Fail quietly with put() method.");
        for(int i = 0; i < (fs.length * 2); i++) {
            if(fs.put(i, i * 10)) System.out.println("fs.put " + (i*10));
            else System.out.println("fs.put fail when i is " + i);
        }

        System.out.println("fail quietly with get() method.");
        for(int i = 0; i < (fs.length*2); i++) {
            x = fs.get(i);
